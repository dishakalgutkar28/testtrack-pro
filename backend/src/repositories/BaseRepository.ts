import {
  DatabaseConnection,
  FilterCriteria,
  FindOptions,
  PaginatedResult
} from '../types';
import { DatabaseError, DuplicateError } from './errors';

/**
 * Base Repository Class
 * All repositories should extend this class
 * Provides CRUD and common database operations with full TypeScript support
 */
class BaseRepository<T extends { id?: any }> {
  protected tableName: string;
  protected db: DatabaseConnection;

  constructor(tableName: string, db: DatabaseConnection) {
    this.tableName = tableName;
    this.db = db;
  }

  /**
   * Find all records with filters, ordering, and pagination
   * @param filters WHERE clause filters {column: value}
   * @param options {orderBy, limit, offset}
   * @returns Promise<Array> Array of records
   */
  async findAll(filters: FilterCriteria = {}, options: FindOptions = {}): Promise<T[]> {
    return new Promise((resolve, reject) => {
      try {
        let query = `SELECT * FROM ${this.tableName}`;
        const values: any[] = [];

        // Build WHERE clause from filters
        const whereConditions: string[] = [];
        for (const [key, value] of Object.entries(filters)) {
          if (value !== null && value !== undefined) {
            whereConditions.push(`${key} = ?`);
            values.push(value);
          }
        }

        if (whereConditions.length > 0) {
          query += ` WHERE ${whereConditions.join(' AND ')}`;
        }

        // Add ordering
        if (options.orderBy) {
          query += ` ORDER BY ${options.orderBy}`;
        }

        // Add pagination
        if (options.limit) {
          query += ` LIMIT ${parseInt(String(options.limit))}`;
          if (options.offset) {
            query += ` OFFSET ${parseInt(String(options.offset))}`;
          }
        }

        this.db.query(query, values, (err: any, results: any) => {
          if (err) {
            reject(new DatabaseError(err.message));
          } else {
            resolve(results || []);
          }
        });
      } catch (error: any) {
        reject(new DatabaseError(error.message));
      }
    });
  }

  /**
   * Find single record by ID
   * @param id Record ID
   * @returns Promise<T|null> Record or null
   */
  async findById(id: any): Promise<T | null> {
    return new Promise((resolve, reject) => {
      this.db.query(
        `SELECT * FROM ${this.tableName} WHERE id = ?`,
        [id],
        (err: any, results: any) => {
          if (err) {
            reject(new DatabaseError(err.message));
          } else {
            resolve(results?.[0] || null);
          }
        }
      );
    });
  }

  /**
   * Find one record matching criteria
   * @param criteria WHERE clause {column: value}
   * @returns Promise<T|null> Single record or null
   */
  async findOne(criteria: FilterCriteria): Promise<T | null> {
    return new Promise((resolve, reject) => {
      try {
        const columns = Object.keys(criteria);
        const values = Object.values(criteria);
        const whereClause = columns.map((col) => `${col} = ?`).join(' AND ');

        this.db.query(
          `SELECT * FROM ${this.tableName} WHERE ${whereClause}`,
          values,
          (err: any, results: any) => {
            if (err) {
              reject(new DatabaseError(err.message));
            } else {
              resolve(results?.[0] || null);
            }
          }
        );
      } catch (error: any) {
        reject(new DatabaseError(error.message));
      }
    });
  }

  /**
   * Create new record
   * @param data Record data {column: value}
   * @returns Promise<T> Created record with inserted ID
   */
  async create(data: Partial<T>): Promise<T> {
    return new Promise((resolve, reject) => {
      try {
        const columns = Object.keys(data);
        const placeholders = columns.map(() => '?').join(', ');
        const values = Object.values(data);

        this.db.query(
          `INSERT INTO ${this.tableName} (${columns.join(', ')}) VALUES (${placeholders})`,
          values,
          (err: any, result: any) => {
            if (err) {
              if (err.code === 'ER_DUP_ENTRY') {
                reject(new DuplicateError('Record already exists'));
              } else {
                reject(new DatabaseError(err.message));
              }
            } else {
              resolve({ id: result.insertId, ...data } as T);
            }
          }
        );
      } catch (error: any) {
        reject(new DatabaseError(error.message));
      }
    });
  }

  /**
   * Update record by ID
   * @param id Record ID
   * @param data Updated fields {column: value}
   * @returns Promise<boolean> True if updated, false otherwise
   */
  async update(id: any, data: Partial<T>): Promise<boolean> {
    return new Promise((resolve, reject) => {
      try {
        const columns = Object.keys(data);
        const updates = columns.map((col) => `${col} = ?`).join(', ');
        const values = [...Object.values(data), id];

        this.db.query(
          `UPDATE ${this.tableName} SET ${updates} WHERE id = ?`,
          values,
          (err: any, result: any) => {
            if (err) {
              reject(new DatabaseError(err.message));
            } else {
              resolve(result.affectedRows > 0);
            }
          }
        );
      } catch (error: any) {
        reject(new DatabaseError(error.message));
      }
    });
  }

  /**
   * Soft delete - mark record as deleted without removing it
   * @param id Record ID
   * @returns Promise<boolean> True if updated, false otherwise
   */
  async softDelete(id: any): Promise<boolean> {
    return this.update(id, { deleted_at: new Date() } as Partial<T>);
  }

  /**
   * Hard delete - permanently remove record from database
   * @param id Record ID
   * @returns Promise<boolean> True if deleted, false otherwise
   */
  async delete(id: any): Promise<boolean> {
    return new Promise((resolve, reject) => {
      try {
        this.db.query(
          `DELETE FROM ${this.tableName} WHERE id = ?`,
          [id],
          (err: any, result: any) => {
            if (err) {
              reject(new DatabaseError(err.message));
            } else {
              resolve(result.affectedRows > 0);
            }
          }
        );
      } catch (error: any) {
        reject(new DatabaseError(error.message));
      }
    });
  }

  /**
   * Count records matching filters
   * @param filters WHERE clause filters {column: value}
   * @returns Promise<number> Count of matching records
   */
  async count(filters: FilterCriteria = {}): Promise<number> {
    return new Promise((resolve, reject) => {
      try {
        let query = `SELECT COUNT(*) as count FROM ${this.tableName}`;
        const values: any[] = [];

        const whereConditions: string[] = [];
        for (const [key, value] of Object.entries(filters)) {
          if (value !== null && value !== undefined) {
            whereConditions.push(`${key} = ?`);
            values.push(value);
          }
        }

        if (whereConditions.length > 0) {
          query += ` WHERE ${whereConditions.join(' AND ')}`;
        }

        this.db.query(query, values, (err: any, results: any) => {
          if (err) {
            reject(new DatabaseError(err.message));
          } else {
            resolve(results?.[0]?.count || 0);
          }
        });
      } catch (error: any) {
        reject(new DatabaseError(error.message));
      }
    });
  }

  /**
   * Paginate results with automatic total count
   * @param filters WHERE clause filters
   * @param page Page number (1-based)
   * @param limit Records per page
   * @returns Promise<PaginatedResult<T>> {data: [], pagination: {...}}
   */
  async paginate(
    filters: FilterCriteria = {},
    page: number = 1,
    limit: number = 20
  ): Promise<PaginatedResult<T>> {
    try {
      const offset = (page - 1) * limit;

      const [data, total] = await Promise.all([
        this.findAll(filters, { limit, offset, orderBy: 'id DESC' }),
        this.count(filters)
      ]);

      const pages = Math.ceil(total / limit);

      return {
        data,
        pagination: {
          total,
          page,
          limit,
          pages,
          hasNext: page < pages,
          hasPrev: page > 1
        }
      };
    } catch (error: any) {
      throw new DatabaseError(error.message);
    }
  }

  /**
   * Execute raw SQL query
   * @param sql SQL query
   * @param values Query parameters
   * @returns Promise<Array> Query results
   */
  async query<R = any>(sql: string, values: any[] = []): Promise<R[]> {
    return new Promise((resolve, reject) => {
      this.db.query(sql, values, (err: any, results: any) => {
        if (err) {
          reject(new DatabaseError(err.message));
        } else {
          resolve(results);
        }
      });
    });
  }
}

export default BaseRepository;
